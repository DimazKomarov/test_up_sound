...
YA_TRACK_RE = re.compile(r"(?:music\.yandex\.[^/]+/album/\d+/track/|music\.yandex\.[^/]+/track/)(\d+)")
...
class YandexMusicService:
    ...
    def get_track_info(self, track_id: str) -> Optional[TrackInfo]:
        ...
        artist_names = ", ".join(track.artists_name()) if track.artists_name() else "Unknown"
...
def extract_track_id(text: str) -> Optional[str]:
    ...
    if parsed and parsed.netloc and "music.yandex." in parsed.netloc:
        ...
        # /track/<track_id>
        if len(parts) >= 4 and parts[0] == "album" and parts[2] == "track" and parts[3].isdigit():
            return parts[3]
    ...
    async def text_handler(message: Message) -> None:
        ...
        try:
            ...
            if "(451)" in error_text or "Unavailable For Legal Reasons" in error_text:
                logging.warning("Ограничение региона/прав на стороне Яндекс.Музыки: %s", error_text)
                await message.answer(
                ...
                await message.answer("Сетевая ошибка при обращении к Яндекс.Музыке. Попробуйте позже.")
            return
        except Exception:
            logging.exception("Ошибка при запросе к yandex-music")
            await message.answer("Не удалось получить данные о треке. Проверьте токен и ссылку.")
            return
